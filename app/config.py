...
SERVICE_NAME = "botduachuot-host-mcp"
VERSION = "1.0.0"
...
# --- BotDuaChuot extensions ---
OSINT_MODE = os.getenv("OSINT_MODE", "false").lower() == "true"
CTF_LIVE_MODE = os.getenv("CTF_LIVE_MODE", "true").lower() == "true"
DRY_RUN_DEFAULT = os.getenv("DRY_RUN_DEFAULT", "false").lower() == "true"
OPSEC_JITTER_MIN_MS = max(0, int(os.getenv("OPSEC_JITTER_MIN_MS", "800")))
OPSEC_JITTER_MAX_MS = max(0, int(os.getenv("OPSEC_JITTER_MAX_MS", "3000")))
GEO_DATASETS_DIR = Path(os.getenv("GEO_DATASETS_DIR", str(BASE_DIR / "datasets"))).resolve()
GEO_REQUIRED_FACTS = max(2, int(os.getenv("GEO_REQUIRED_FACTS", "2")))
GEO_MAX_LANDMARK_MATCHES = max(1, int(os.getenv("GEO_MAX_LANDMARK_MATCHES", "5")))