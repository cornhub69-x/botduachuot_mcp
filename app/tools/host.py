...
@mcp.tool(
    name="duachuot_list_directory",
    description=(
        "List files and directories on the host machine under the configured "
        "HOST_WORKSPACE_DIR policy."
    ),
)
def duachuot_list_directory(path: str = ".", max_entries: int = 500) -> dict[str, Any]:
    try:
...
@mcp.tool(
    name="duachuot_append_file",
    description="Append UTF-8 text to a file on the host machine.",
)
def duachuot_append_file(path: str, content: str) -> dict[str, Any]:
    try:
...
@mcp.tool(
    name="duachuot_make_directory",
    description="Create a directory on the host machine under HOST_WORKSPACE_DIR policy.",
)
def duachuot_make_directory(path: str, parents: bool = True) -> dict[str, Any]:
    try:
...
def duachuot_check_command(command: str) -> dict[str, Any]:
    try:
