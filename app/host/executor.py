...
from app.platform import probe_platform
...
def _shell_invocation(command: str) -> list[str]:
    """Choose the shell invocation for the current platform.

    Linux/macOS: bash --noprofile --norc -c. Windows: powershell/cmd via
    COMSPEC when WSL is unavailable, otherwise WSL bash bridge.
    """
    probe = probe_platform()
    if probe["windows"]:
        if probe["wsl_available"]:
            return ["wsl.exe", "bash", "--noprofile", "--norc", "-c", command]
        shell = os.environ.get("COMSPEC", "cmd.exe")
        if shell.lower().endswith("powershell.exe") or "powershell" in shell.lower():
