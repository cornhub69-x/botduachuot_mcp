...
def _add_global_options(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--version", action="version", version=f"duachuot {VERSION}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = CLIArgumentParser(
        prog="duachuot",
        description="Operate BotDuaChuot Host MCP and call its REST API.",
    )
    ...
    geo = commands.add_parser("geo", help="Offline Geo Engine operations")
    geo_commands = geo.add_subparsers(dest="geo_command", required=True)
    geo_convert = geo_commands.add_parser("convert", help="Convert coordinates (DMS/decimal/UTM/MGRS)")
    geo_convert.add_argument("--lat", type=float)
    geo_convert.add_argument("--lon", type=float)
    geo_convert.add_argument("--dms-lat")
    geo_convert.add_argument("--dms-lon")
    geo_convert.add_argument("--ref", default="", help="N/S/E/W suffix for DMS inputs")
    geo_convert.add_argument("--utm-zone", type=int)
    geo_convert.add_argument("--hemisphere", default="N", choices=["N", "S"])
    geo_convert.add_argument("--easting", type=float)
    geo_convert.add_argument("--northing", type=float)
    geo_convert.add_argument("--datum-src", default="WGS84")
    geo_convert.add_argument("--datum-dst", default="WGS84")
    geo_reverse = geo_commands.add_parser("reverse", help="Offline landmark reverse geocoding")
    geo_reverse.add_argument("lat", type=float)
    geo_reverse.add_argument("lon", type=float)
    geo_reverse.add_argument("--limit", type=int, default=5)
    geo_verify = geo_commands.add_parser("verify", help="Require >=2 independent facts before concluding")
    geo_verify.add_argument("lat", type=float)
    geo_verify.add_argument("lon", type=float)

    ops = commands.add_parser("ops", help="OPSEC gate: check commands/targets, get jitter")
    ops_commands = ops.add_subparsers(dest="ops_command", required=True)
    ops_check = ops_commands.add_parser("check", help="Inspect a command or remote target")
    ops_check.add_argument("target")
    ops_check.add_argument("--kind", default="command", choices=["command", "remote"])
    ops_commands.add_parser("jitter", help="Suggested delay before the next remote request")

    platform_parser = commands.add_parser("platform", help="Probe host platform and tool availability")
    platform_parser.add_argument("--tool", help="Check availability of a specific tool")
