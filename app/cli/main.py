...
from app.cli.commands.investigation import (
    handle_geo,
    handle_ops,
    handle_platform,
)
...
def _dispatch(ctx: CLIContext, args) -> int:
    ...
    if command == "geo":
        return handle_geo(ctx, args)
    if command == "ops":
        return handle_ops(ctx, args)
    if command == "platform":
        return handle_platform(ctx, args)
    if command == "completion":
        return handle_completion(ctx, args)
    if command == "version":
        payload = {"ok": True, "cli": "duachuot", "version": VERSION, "service_version": VERSION}
        if ctx.json_output:
            emit_json(payload)
        else:
            print(f"duachuot {VERSION}")
        return 0
