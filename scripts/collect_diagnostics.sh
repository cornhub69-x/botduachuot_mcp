...
.venv/bin/duachuot version > "$OUTPUT_DIR/version.txt"
.venv/bin/duachuot status --json > "$OUTPUT_DIR/status.json" || true
.venv/bin/duachuot config show --json > "$OUTPUT_DIR/config-redacted.json"
.venv/bin/duachuot config validate --json > "$OUTPUT_DIR/config-validation.json" || true
.venv/bin/duachuot doctor --local-only --json > "$OUTPUT_DIR/doctor-local.json" || true
...
Configuration output is generated through `duachuot config show` and is redacted.
EOF
